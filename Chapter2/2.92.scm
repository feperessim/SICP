...
(define (install-polynomial-package)
    ...
    (or (empty-termlist? (term-list p))
        (and (=zero? (coeff
             ...
             (=zero-poly? (make-poly
                           ...
                           (rest-terms
                            (term-list p)))))))
  (define (coerce-poly-to-var p-sub p-main)
    (let ((var (variable p-main)))
      (make-poly var
                 (adjoin-term
                  (make-term 0 (tag p-sub))
                  (empty-termlist-of-type (term-list p-main))))))
  (define (add-poly p1 p2)
    (cond 
     ...
     ((variable>? (variable p1) (variable p2))
      (add-poly p1 (coerce-poly-to-var p2 p1)))
     
     (else
      (add-poly (coerce-poly-to-var p1 p2) p2))))
     ...
     ((same-variable? (variable p1) (variable p2))
      (make-poly (variable p1)
                 (mul-terms (term-list p1)
                            (term-list p2))))
     ((variable>? (variable p1) (variable p2))
      (mul-poly p1 (coerce-poly-to-var p2 p1)))
     (else
      (mul-poly (coerce-poly-to-var p1 p2) p2))))
  ...
  (define (add-poly-to-scheme-number p n)
    (let ((var (variable p))) 
      (let ((poly-n
             (make-poly var
                        (adjoin-term
        ...
        (add-poly p poly-n))))
  (define (mul-poly-to-scheme-number p n)
    (let ((var (variable p))) 
      (let ((poly-n
             (make-poly var
                        (adjoin-term
  ...
  (put 'add '(polynomial scheme-number)
       (lambda (p n) (tag (add-poly-to-scheme-number p n))))
  (put 'add '(scheme-number polynomial)
       (lambda (n p) (tag (add-poly-to-scheme-number p n))))
  (put 'mul '(polynomial polynomial)
       (lambda (p1 p2) 
         (tag (mul-poly p1 p2))))
  (put 'mul '(polynomial scheme-number)
       (lambda (p n) (tag (mul-poly-to-scheme-number p n))))
  (put 'mul '(scheme-number polynomial)
       (lambda (n p) (tag (mul-poly-to-scheme-number p n))))
  (put 'make 'polynomial
       (lambda (var terms) 
...
(define variable-priority-table (make-hash-table))

(define (put-variable-priority var priority)
  (hash-set! variable-priority-table var priority))

(define (get-variable-priority var)
  (hash-ref variable-priority-table var 0))

(define (variable>? v1 v2)
  (> (get-variable-priority v1)
     (get-variable-priority v2)))
...
(put-variable-priority 'x 1)
(put-variable-priority 'y 0)
...
(assert-equ (mul p3 p2)
            p3-mul-p2
            "Mul polynomials in differen variables produces treats on of the polynomials as a constant")

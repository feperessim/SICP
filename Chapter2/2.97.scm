  ...
  (define (reduce-terms n d)
    (let* ((gcd-n-d (gcd-terms n d))
           ...
           (O2 (order (first-term gcd-n-d)))
            ;;(newline)(display "C:")(write (cdr (coeff c))))(newline)
           (factor (expt (cdr c) (+ 1 (- O1 O2))))
           (mul-factor-n
            (mul-term-by-all-terms
             (make-term 0 (make-scheme-number factor))
             n))
           (mul-factor-d
            (mul-term-by-all-terms
             (make-term 0 (make-scheme-number factor))
             d))
           (div-mul-factor-n-by-gcd
            (car
             (div-terms mul-factor-n
                        gcd-n-d)))
           (div-mul-factor-d-by-gcd
            (car
             (div-terms mul-factor-d
                        gcd-n-d)))
           (gcd-coeffs-n
            (gcd-coeffs (coeff
                         (first-term div-mul-factor-n-by-gcd))
                        (rest-terms div-mul-factor-n-by-gcd)))
           (gcd-coeffs-d
            (gcd-coeffs (coeff
                         (first-term div-mul-factor-d-by-gcd))
                        (rest-terms div-mul-factor-d-by-gcd)))
           (final-gcd-n-d
            (greatest-common-divisor gcd-coeffs-n
                                     gcd-coeffs-d))
           (nn
            (car
             (div-terms div-mul-factor-n-by-gcd                 
                        (adjoin-term (make-term 0 final-gcd-n-d)
                                     (empty-termlist-of-type
                                      div-mul-factor-n-by-gcd)))))
           (dd
            (car
             (div-terms div-mul-factor-d-by-gcd                 
                        (adjoin-term (make-term 0 final-gcd-n-d)
                                     (empty-termlist-of-type
                                      div-mul-factor-d-by-gcd))))))
      (list nn dd)))
...
(define polynomial-fraction (add rf1 rf2))

(define polynomial-fraction-reduced
  (make-rational
   (make-polynomial 'x
                    (make-sparse-term-list
                     (list (make-term 3 (num -1))
...
(assert-equ polynomial-fraction
            polynomial-fraction-reduced            
            "A fraction of polinomials are reduced to the lowest terms")
...
(assert-equ rf3
            rf3-reduced
            "A fraction of scheme numbers are reduced to the lowest terms")
